...
void sumaMatriz(int, int);

void main()
{
    float matriz[][tam] = {{5.4, 3.2}, {7.8, 2.1}, {5.4, 1.2}};
    int op = 0, fil, colum, fil2, colum2;
...
void sumaMatriz(int fila, int columna)
{
    int i = 0, j = 0;
    float arreglo1[fila][columna], arreglo2[fila][columna], suma[fila][columna];
    ...
    while(i < fila)
    {
        ...
        while(j < columna)
        {
            suma[i][j] = arreglo2[i][j] + arreglo1[i][j];
            printf("%.2f\t", suma[i][j]);
